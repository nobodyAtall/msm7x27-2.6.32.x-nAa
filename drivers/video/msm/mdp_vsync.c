...
#define MDP_SYNC_0		0x100
#define MDP_SYNC_STATUS_0	0x10c
#define MDP_SYNC_1		0x104
...
#ifdef MDP_HW_VSYNC
static void mdp_set_sync_0(struct msm_fb_data_type *mfd, int vsync_cnt)
{
	unsigned long cfg;
	...
	MDP_OUTP(MDP_BASE + MDP_SYNC_0, cfg);
}

#ifdef CONFIG_FB_MSM_MDP40
static void mdp_set_sync_1(struct msm_fb_data_type *mfd, int vsync_cnt)
{
	unsigned long cfg;
	...
	MDP_OUTP(MDP_BASE + MDP_SYNC_1, cfg);
}
#endif
#endif
...
void mdp_config_vsync(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->panel_info.lcd.vsync_enable) {
		...
		if (mfd->use_mdp_vsync) {
			uint32 vsync_cnt_cfg, vsync_cnt_dem;
			...
			if (mdp_vsync_clk_speed_hz == 0) {
				...
				mdp_set_sync_0(mfd, vsync_cnt_cfg);


#ifdef CONFIG_FB_MSM_MDP40
				if (mdp_hw_revision < MDP4_REVISION_V2_1)
					mdp_set_sync_1(mfd, vsync_cnt_cfg);
#endif
...
void mdp_vsync_config_update(struct msm_panel_info *pinfo)
{
	...
	mdp_vsync_clk_speed_hz = clk_get_rate(mdp_vsync_clk);
	vsync_cnt_dem = (pinfo->lcd.refx100 * total_line) / 100;
	vsync_cnt_cfg = (mdp_vsync_clk_speed_hz) / vsync_cnt_dem;

	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	cfg = readl(MDP_BASE + MDP_SYNC_0);
	cfg &= (0xfff80000);
	cfg |= vsync_cnt_cfg;
	MDP_OUTP(MDP_BASE + MDP_SYNC_0, cfg);
#ifdef CONFIG_FB_MSM_MDP40
